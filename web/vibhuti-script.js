...
function handleReviewSubmit(event) {
  ...
  const course = document.getElementById('rev-course').value;
  ...
  // Varied avatar colors for a professional look
  const avatarColors = [
    'linear-gradient(135deg, #b8860b, #8b6000)', // Gold/Amber
    'linear-gradient(135deg, #4a3728, #2d2419)', // Earthy
    'linear-gradient(135deg, #d4a847, #b8860b)', // Light Gold
    'linear-gradient(135deg, #5d4a38, #2b2015)'  // Muted Brown
  ];
  ...
  setTimeout(() => {
    ...
    newReview.style.borderLeft = '4px solid var(--golden-primary)'; // Distinctive highlight for new reviews
    
    newReview.innerHTML = `
      <div class="review-item-header">
        <div class="reviewer-info">
          <div class="reviewer-avatar" style="background: ${randomColor}">${initials}</div>
          <div class="reviewer-meta">
            <h4>${name}</h4>
            <span class="review-course-tag">${course}</span>
          </div>
        </div>
  ...
  }, 800); // Small delay for "expertise" feel (processing simulation)
}

function showToast(message, type = "info") {
  const toast = document.createElement('div');
  toast.className = 'copy-notification'; // Reusing existing premium toast style
  toast.style.background = type === 'success' ? '#d4f1d4' : '#f5d794';
  toast.style.color = type === 'success' ? '#1b4332' : '#1a1442';
